# =============================================================================
#          By : Mohamed T. Ismail
#      Course : Getting and Cleaning Data (Johns Hopkins University)
#    Provider : Coursera.com
# Description : Week 4 Practice - Editing Text Variables, Regex, Dates
# =============================================================================


# Question 1 ------------------------------------------------------------------
#
# The American Community Survey distributes downloadable data about U.S 
# communities. Download the 2006 microdata survey about housing for the state 
# of Idaho using download.file() from here: 
#
#   https://d396qusza40orc.cloudfront.net/getdata%2Fdata%2Fss06hid.csv 
#
# The code book, describing the variable names is here: 
#
#   https://d396qusza40orc.cloudfront.net/getdata%2Fdata%2FPUMSDataDict06.pdf 
#
# Apply strsplit() to split all the names of the data frame on the characters 
# "wgtp". What is the value of the 123 element of the resulting list?
#
# -----------------------------------------------------------------------------

question1 <- function()
{
	housingData <- read.csv( "./data/idaho_housing.csv" )

	splitNames = strsplit( names(housingData), "wgtp" )
	splitNames[123]
}



# Question 2 ------------------------------------------------------------------
#
# Load the Gross Domestic Product data for the 190 ranked countries : 
#
#       https://d396qusza40orc.cloudfront.net/getdata%2Fdata%2FGDP.csv 
#
# Remove the commas from the GDP numbers in millions of dollars and 
# average them. What is the average? 
#
# -----------------------------------------------------------------------------

question2 <- function()
{
	## Read and cleanup GDP Data
	gdp_data <- read.csv( "./data/GDP.csv" )
	gdpData  <- gdp_data[5:194,]
	gdp      <- cbind( as.character(gdpData$X), 
                         as.character(gdpData$Gross.domestic.product.2012), 
                         as.character(gdpData$X.2), 
                         as.character(gdpData$X.3) )
	colnames(gdp) <- c("CountryCode", "Ranking", "Country", "Value" )
      
      ## Remove spaces and commas in value column
	gdp[,4] = gsub( " ", "", gdp[,4] )   
	gdp[,4] = gsub( ",", "", gdp[,4] )   
      
      ## Convert strings to ints and get the mean
      values <- as.numeric( gdp[,4] )
      mean( values )
}



# Question 3 ------------------------------------------------------------------
#
# In the data set from Question 2 what is a regular expression that would 
# allow you to count the number of countries whose name begins with "United"? 
# Assume that the variable with the country names in it is named countryNames. 
# How many countries begin with United?
#
# -----------------------------------------------------------------------------

question3 <- function()
{
      ## Read and cleanup GDP Data
      gdp_data <- read.csv( "./data/GDP.csv" )
      gdpData  <- gdp_data[5:194,]
      gdp      <- cbind( as.character(gdpData$X), 
                         as.character(gdpData$Gross.domestic.product.2012), 
                         as.character(gdpData$X.2), 
                         as.character(gdpData$X.3) )
      colnames(gdp) <- c("CountryCode", "Ranking", "Country", "Value" )

      ## try some regular expression
      
      print( grep("*United", gdp[,3] ) )
      print( grep("^United", gdp[,3] ) )
      print( grep("^United", gdp[,3] ) ) 
      print( grep("*United", gdp[,3] ) )
}



# Question 4 ------------------------------------------------------------------
#
# Load the Gross Domestic Product data for the 190 ranked countries: 
# 
#   https://d396qusza40orc.cloudfront.net/getdata%2Fdata%2FGDP.csv 
#
# Load the educational data from this data set: 
#
#   https://d396qusza40orc.cloudfront.net/getdata%2Fdata%2FEDSTATS_Country.csv 
# 
# Match the data based on the country shortcode. Of the countries for which 
# the end of the fiscal year is available, how many end in June? 
#
# -----------------------------------------------------------------------------

question4 <- function()
{
      ## Read and cleanup GDP Data
      gdp_data <- read.csv( "./data/GDP.csv" )
      gdpData  <- gdp_data[5:194,]
      gdp      <- cbind( as.character(gdpData$X), 
                         as.character(gdpData$Gross.domestic.product.2012), 
                         as.character(gdpData$X.2), 
                         as.character(gdpData$X.3) )
      colnames(gdp) <- c("CountryCode", "Ranking", "Country", "Value" )
      
      ## Read education data
      edustats <- read.csv( "./data/EducationStats.csv" )
      
      ## Merge the data
      allData <- merge( gdp, edustats, by="CountryCode" )

      ## Search the Special.Notes column for "Fiscal Year End" and "June"
      fiscalYearData <- allData[ grepl( "Fiscal" , allData$Special.Notes ), ]
      endsJune <- grep( "June" , fiscalYearData$Special.Notes, value=TRUE )
      endsJune
	
}



# Question 5 ------------------------------------------------------------------
#
# You can use the quantmod (http://www.quantmod.com/) package to get 
# historical stock prices for publicly traded companies on the NASDAQ and NYSE. 
# Use the following code to download data on Amazon's stock price and get the 
# times the data was sampled.
#
#      library(quantmod)
#      amzn = getSymbols("AMZN",auto.assign=FALSE)
#      sampleTimes = index(amzn) 
#
# How many values were collected in 2012? 
# How many values were collected on Mondays in 2012?
#
# -----------------------------------------------------------------------------

question5 <- function()
{
      library(quantmod)
      amzn = getSymbols("AMZN",auto.assign=FALSE)
      sampleTimes = index(amzn)
      
      # Get values from 2012
      samplesIn2012 <- grep( "2012" , sampleTimes, value=TRUE )

      # Get values collected on Mondays in 2012
      library( lubridate )
      dates    <- ymd( samplesIn2012 )
      weekdays <- wday( dates, label=TRUE )
	count( grep( "Mon", weekdays, value=TRUE ) ) 
      
}


